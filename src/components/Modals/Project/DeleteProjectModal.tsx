...
function DeleteProjectModal({ className, children, projectId, ...props }: DeleteProjectModalProps) {
	...
	return (
		<Modal isActive={props.isActive}>
			<ModalHeader close={props.close}>Are you sure?</ModalHeader>
			<ModalBody>This will delete the project</ModalBody>
			<ModalFooter className="flex gap-2">
				<Button onClick={props.close}>Cancel</Button>
				<Button onClick={() => deleteProjectMutation.mutate()}>Yes</Button>
			</ModalFooter>
		</Modal>
	);
}
