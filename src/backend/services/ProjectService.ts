...
const fetchProjects = async () => {
	...
	await Promise.all(projects.map(async project => {
		return Promise.all([updateProjectMetaData(project), upsertProjectCommits(project), upsertProjectLanguages(project)])
	}));
}
...
const upsertProjectCommits = async (project: Project) => {
	...
	if (response.headers.link) {
		...
		await Promise.all(new Array(Number(parsedLink.last.page) - 1).fill("").map(async (item, index) => {
			const response = await octokit.rest.repos.listCommits({
				owner: config.GITHUB_REPO_OWNER,
				repo: project.name,
				per_page: 100,
